...
- **Map**: create an issue with label `wayfinder:map` (scope `wayfinder`, name `map`).
- **Child ticket**: create an issue with `parent` set to the map's number and a `wayfinder:<type>` label (scope `wayfinder`, name matching the type).
