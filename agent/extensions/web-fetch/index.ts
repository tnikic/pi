...
async function ensureBrowser(): Promise<NonNullable<typeof browser>> {
	if (!browser?.isConnected()) {
		browser = await chromium.launch({ headless: true });
	}
	return browser as NonNullable<typeof browser>;
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptSnippet:
			"Fetch a web page, render JavaScript, and return markdown content",
